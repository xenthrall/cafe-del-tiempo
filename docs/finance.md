...
- **`categories`**: `id`, `name`, `type` (enum `CategoryType`: `income`, `expense`), `parent_id` (auto-referencia nullable, para la jerarquía tipo `Transporte > Combustible`), `financial_context_id` (nullable — a qué contexto pertenece la categoría, para poder ver de un vistazo qué categorías corresponden a qué contexto).
...
- **2026-09-21 — Contextos archivables**: `financial_contexts.is_active` (default `true`). Pensado para que el usuario pueda archivar un contexto que ya no usa sin perder su histórico — a nivel de datos y modelo únicamente por ahora (`FinancialContext::$fillable`/cast); falta el lado UI: que los selectores de movimiento oculten los contextos inactivos y que exista una forma de archivar/reactivar desde `ManageFinancialContexts`.
- **2026-09-21 — Borrado protegido de cuentas y contextos, a nivel de base de datos**: `movements.account_id`/`from_account_id`/`to_account_id` y `movements.financial_context_id` pasan de `cascadeOnDelete()`/`nullOnDelete()` a **`restrictOnDelete()`** — la base de datos ahora rechaza borrar una cuenta o un contexto financiero que tenga movimientos asociados, en vez de borrar en cascada o poner en null en silencio. `categories.category_id` en movimientos sigue en `nullOnDelete()` sin cambios (ver "Borrado de cuentas protegido" en "Lógica de negocio"). Importante: como estas migraciones se modificaron directamente (no se creó una nueva), una base de datos ya migrada con el esquema anterior necesita un `migrate:fresh` (o equivalente) para recibir el cambio.
...
- **Borrado protegido de cuentas y contextos**: `Account::hasMovements()`/`FinancialContext::hasMovements()` existen para que la UI pueda avisar *antes* de intentar borrar — perder una cuenta o un contexto con movimientos borraría histórico real. El respaldo real ya no es un `cascadeOnDelete()` silencioso: es `restrictOnDelete()` a nivel de base de datos (ver "Modelo de datos"), que directamente rechaza el borrado. Tanto `ManageAccounts` como `ManageFinancialContexts` usan `hasMovements()` antes de borrar (este último con una notificación explicando por qué no se pudo). Categorías sí se pueden borrar libremente: sus movimientos quedan sin categoría (`nullOnDelete`) en vez de perderse.
