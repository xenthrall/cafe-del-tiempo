...
- **Borrado protegido de cuentas y contextos**: `Account::hasMovements()`/`FinancialContext::hasMovements()` existen para que la UI pueda avisar *antes* de intentar borrar — perder una cuenta o un contexto con movimientos borraría histórico real. El respaldo real ya no es un `cascadeOnDelete()` silencioso: es `restrictOnDelete()` a nivel de base de datos (ver "Modelo de datos"), que directamente rechaza el borrado. `ManageAccounts` ya usa `hasMovements()` antes de borrar; `ManageFinancialContexts` todavía no (pendiente, ver "Modelo de datos" — Contextos archivables). Categorías sí se pueden borrar libremente: sus movimientos quedan sin categoría (`nullOnDelete`) en vez de perderse.
...
- **`ManageMovements`** (`/app/movements`): pestañas por tipo (Todos/Ingresos/Gastos/Transferencias/Ajustes), crear/editar/eliminar vía `Actions/ManageMovementAction` y `Actions/DeleteMovementAction` (acciones de Filament reutilizables — ver "Acciones y tabla de movimientos" abajo), y el listado sí usa el componente **Table** de Filament (`Tables/MovementsTable`), con paginación y filtros reales. Esto revierte, solo para el listado de movimientos, la parte de la decisión de arriba que excluía las tablas genéricas — el resto del módulo (formularios, páginas) sigue sin usarlas.
- **`ManageCategories`** (`/app/categories`): pestañas Gastos/Ingresos, jerarquía padre → hijo, modal de alta/edición, todo a mano (Livewire "a mano" + `x-filament::button`/`x-filament::modal`/`x-filament::tabs`, `Illuminate\Validation\Validator`), sin tabla ni acciones de Filament todavía.
- **`ManageFinancialContexts`** (`/app/financial-contexts`): listado simple con conteo de movimientos por contexto, también a mano.
...
### Acciones y tabla de movimientos — decidido

- **`Filament\Traits\HidesPageHeader`**: oculta el encabezado (título + migas de pan) que Filament renderiza por defecto en una página, para que el módulo se sienta menos "panel de administración". Se aplica en `ManageMovements`; cada página que lo use debe darle su propio contexto en el Blade (ver el bloque de icono + `<h1>` propio al inicio de `manage-movements.blade.php`).
